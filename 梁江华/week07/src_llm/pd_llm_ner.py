"""
使用大模型 API 做 NER：Peoples Daily 版本的 zero-shot vs few-shot 对比
...
使用方式：
  python src_llm/pd_llm_ner.py
  python src_llm/pd_llm_ner.py --n_samples 50 --model qwen-max
...
def _record_text(record: dict) -> str:
    return "".join(record.get("tokens") or [])


def gold_spans_from_record(record: dict) -> set[tuple[str, str, int, int]]:
    """从 BIO 标签提取 gold spans，格式：{(surface, type, start, end)}。"""
    spans = set()
    ...
    while i < n:
        ...
        if isinstance(tag, str) and tag.startswith("B-"):
            ...
            spans.add(("".join(entity_tokens), etype, start, end))
            continue
        if isinstance(tag, str) and tag.startswith("I-"):
            etype = tag[2:]
            spans.add((tokens[i], etype, i, i))
        i += 1
    return spans
...
输出格式（严格遵守，不要包含其他文字）：
{"entities": [{"text": "实体文本", "type": "PER/ORG/LOC"}, ...]}
...
FEW_SHOT_EXAMPLES = [
    ...
    {
        "text": "任正非在深圳会见华为技术有限公司员工。",
        "output": '{"entities": [{"text": "任正非", "type": "PER"}, {"text": "深圳", "type": "LOC"}, {"text": "华为技术有限公司", "type": "ORG"}]}'
    },
    {
        "text": "中国人民大学位于北京，张三在该校任教。",
        "output": '{"entities": [{"text": "中国人民大学", "type": "ORG"}, {"text": "北京", "type": "LOC"}, {"text": "张三", "type": "PER"}]}'
    },
]
...
def sample_records(n: int, seed: int = 42) -> list[dict]:
    ...
    for r in records:
        tags = r.get("ner_tags") or []
        for etype in {tag[2:] for tag in tags if isinstance(tag, str) and tag.startswith(("B-", "I-"))}:
...
def main():
    ...
    out_path = LOG_DIR / "eval_pd_llm.json"
...
def parse_args():
    parser = argparse.ArgumentParser(description="Peoples Daily LLM zero-shot/few-shot NER 对比")
