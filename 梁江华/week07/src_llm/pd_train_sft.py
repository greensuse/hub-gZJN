"""
LLM SFT（监督微调）训练脚本 — Peoples Daily 版本
...
教学重点：
  ...
  3. LoRA 高效微调：相对 3B 基座仅训练少量适配参数，与全量微调对比（--full_ft 开关）
  4. 生成式 NER vs 序列标注（BERT+CRF）：各自的优劣和适用场景

使用方式：
  python src_llm/pd_train_sft.py                        # LoRA，全量训练数据（默认）
  python src_llm/pd_train_sft.py --num_train 2000       # LoRA，2000 条快速演示
  python src_llm/pd_train_sft.py --epochs 1             # 快速验证流程

  # 全量微调（3.09B 参数，需较大显存）
  python src_llm/pd_train_sft.py --full_ft --lr 2e-5
...
def record_to_target(record: dict) -> str:
    """把 Peoples Daily BIO 格式转为 SFT 目标 JSON 字符串。"""
    ...
    while i < n:
        ...
        if isinstance(tag, str) and tag.startswith("B-"):
            ...
            if etype in ENTITY_TYPES:
                entities.append({"text": "".join(span_tokens), "type": etype})
            continue

        # 容错：孤立 I- 标签按单 token 实体处理
        if isinstance(tag, str) and tag.startswith("I-"):
            etype = tag[2:]
            if etype in ENTITY_TYPES:
                entities.append({"text": str(tokens[i]), "type": etype})
        i += 1
...
class SFTDataset(Dataset):
    """
    把 Peoples Daily NER 数据转换为 chat-format SFT 训练样本。
    ...
    def __getitem__(self, idx):
        ...
        prompt_text = self.tokenizer.apply_chat_template(
            [
                ...
                {"role": "user",   "content": _record_text(item)},
            ],
            tokenize=False,
...
def parse_args():
    parser = argparse.ArgumentParser(description="Peoples Daily LLM SFT NER 训练（LoRA / 全量微调）")
    ...
    parser.add_argument("--full_ft",     action="store_true",
                        help="全量微调：跳过 LoRA，更新全部 3.09B 参数（需较大显存）")
    # LoRA 超参（full_ft 时忽略）
...
def main():
    ...
    if tokenizer is None:
        raise RuntimeError("Tokenizer 加载失败，请检查 --model_path")
    if tokenizer.pad_token_id is None:
        if tokenizer.eos_token_id is None:
            raise ValueError("tokenizer 缺少 eos_token_id，无法设置 pad_token_id")
    ...
    if model is None:
        raise RuntimeError("Model 加载失败，请检查 --model_path")
    ...
    print(f"\n下一步：python src_llm/pd_evaluate_sft.py 查看 entity F1 与多方对比")
