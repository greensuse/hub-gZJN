"""
加载 Peoples Daily SFT checkpoint（LoRA / 全量微调），在验证集上评估 NER entity-level F1，
与 BERT+CRF 和 LLM API（zero/few-shot）多方对比

教学重点：
  1. 生成式 NER 的评估方式：JSON 解析 → span-level F1（与 pd_llm_ner.py 一致）
  2. LoRA adapter 自动识别：目录含 adapter_config.json → LoRA，否则 → 全量
  3. 与 BERT+CRF 的对比：生成式 vs 序列标注，各有什么优劣

使用方式：
  python src_llm/pd_evaluate_sft.py                                        # 评估 LoRA 模型（默认）
  python src_llm/pd_evaluate_sft.py --ckpt_dir outputs/pd_sft_full_ckpt   # 评估全量微调模型
  python src_llm/pd_evaluate_sft.py --n_samples 50 --demo                  # 5 条示例快速演示
...
def load_model(model_path: str, ckpt_dir: str, device: torch.device):
    ...
    if is_lora:
        ...
        expected_base = str(adapter_cfg.get("base_model_name_or_path") or "").strip()
        if expected_base:
            expected_path = Path(expected_base)
            if expected_path.exists():
                resolved_model_path = expected_path.resolve()
            else:
                local_candidate = ROOT / "pretrain_models" / expected_path.name
                if local_candidate.exists():
                    resolved_model_path = local_candidate.resolve()
                elif Path(model_path).name != expected_path.name:
                    raise FileNotFoundError(
                        f"LoRA adapter 期望 base model 为: {expected_path.name}，"
                        f"但当前 --model_path 是: {Path(model_path).name}。\n"
                        f"且未找到本地目录: {local_candidate}\n"
                        "请下载对应 base model，或传入匹配的 --model_path。"
                    )

        print(f"检测到 LoRA adapter，加载 base model: {resolved_model_path}")
        tokenizer  = AutoTokenizer.from_pretrained(
            str(resolved_model_path), trust_remote_code=True
        )
...
def gold_spans_from_record(record: dict) -> set[tuple[str, str, int, int]]:
    """从 Peoples Daily BIO 标签提取 gold spans，格式 (surface, type, start, end)。"""
    spans = set()
    tokens = record.get("tokens") or []
    tags   = record.get("ner_tags") or ["O"] * len(tokens)
    ...
    while i < n:
        ...
        if isinstance(tag, str) and tag.startswith("B-"):
            etype = tag[2:]
            span_tokens = [str(tokens[i])]
            start = i
            i += 1
            while i < n and tags[i] == f"I-{etype}":
                span_tokens.append(str(tokens[i]))
                i += 1
            spans.add(("".join(span_tokens), etype, start, i - 1))
            continue
        if isinstance(tag, str) and tag.startswith("I-"):
            etype = tag[2:]
            spans.add((str(tokens[i]), etype, i, i))
        i += 1
    return spans
...
def parse_args():
    parser = argparse.ArgumentParser(description="Peoples Daily LLM SFT NER 评估")
    ...
    parser.add_argument("--n_samples",   default=100, type=int,
                        help="验证集采样数（与 pd_llm_ner.py 默认 100 条对齐）")
...
def main():
    ...
    if not ckpt_dir.exists():
        ...
        print("请先运行 pd_train_sft.py 完成训练。")
    ...
    llm_zero_f1   = "?"
    llm_few_f1    = "?"
    # 对齐 pd_evaluate.py 和 pd_llm_ner.py 生成的日志文件名
    crf_log_path  = LOG_DIR / "eval_pd_crf_validation.json"
    llm_log_path  = LOG_DIR / "eval_pd_llm.json"
    ...
    print(f"\n{'='*65}")
    print(f"Peoples Daily LLM SFT NER 评估结果")
    print(f"{'='*65}")
...
评估标准说明：
  SFT（本脚本）和 LLM API（pd_llm_ner.py）均使用 span F1：
    text.find() 近似定位 → (text, type, start, end) 4元组 → 与 gold 做集合交集
  BERT+CRF（pd_evaluate.py）使用 seqeval：
...
""")
    ...
    # ── 保存结果 ──────────────────────────────────────────────────────────────
    out_path = LOG_DIR / "eval_pd_sft.json"
