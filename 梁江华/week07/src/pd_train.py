...
BERT NER 训练脚本（Peoples Daily 版本）
...
使用方式：
  python src/pd_train.py                        # 训练 BERT+Linear（基线）
  python src/pd_train.py --use_crf              # 训练 BERT+CRF
  python src/pd_train.py --epochs 5 --lr 3e-5   # 自定义超参数
...
def main():
    ...
    log_path = LOG_DIR / f"train_{artifact_tag}.json"
