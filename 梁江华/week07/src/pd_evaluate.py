"""
在测试集上评估 BERT NER 模型（Peoples Daily 版本），并统计非法序列
...
使用方式：
  python src/pd_evaluate.py                        # 评估 BERT+Linear
  python src/pd_evaluate.py --use_crf              # 评估 BERT+CRF
  python src/pd_evaluate.py --split test           # 在测试集上评估
"""
...
def main():
    ...
    artifact_tag = f"pd_{run_tag}"
    ckpt_path = CKPT_DIR / f"best_{artifact_tag}.pt"

    if not ckpt_path.exists():
        print(f"找不到 checkpoint：{ckpt_path}")
        print(f"请先运行：python src/pd_train.py {'--use_crf' if args.use_crf else ''}")
        return
    ...
    out_path = LOG_DIR / f"eval_{artifact_tag}_{split_name}.json"
...
def parse_args():
    ...
    parser.add_argument("--split", choices=["validation", "test"], default="validation",
                        help="Peoples Daily validation 或 test 集均有标签，均可使用")
